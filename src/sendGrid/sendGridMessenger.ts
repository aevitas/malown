...
export class SendGridMessenger implements IEmailMessenger {
    ...
    public constructor(
        @inject(Types.SendGridOptions) options: ISendGridOptions
    ) {
        if (!options) {
            throw new Error(
    ...
    public async sendMessageAsync(
        ...
        return new Promise<IMessageSendResult>(
            async (resolve, reject) => {
                try {
                    await sendGridMailer.send({
                        ...
                        text: !this.useHtmlMessages
                            ? message.body
                            : undefined,
                        html: this.useHtmlMessages
                            ? message.body
                            : undefined
                    });
                    ...
                    return reject(
                        `An error occurred while sending a message: ${err}`
                    );
                }
            }
        );
